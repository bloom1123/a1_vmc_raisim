...
VectorXd LocomotionController::getAction(bool mpc_step, std::vector<double> weights, double mass, vector<double> inertia) {
    ...
    if (mpc_step) {
        stance_action = stance_controller->getAction(weights, mass, inertia);
    }
    else {
